...
class MLP:
    ...
    def forward(self, X):
        ...
        self.output = self.sigmoid(np.dot(self.hidden_output2, self.weights_output) + self.bias_output)
        return self.output
    ...
    def backward(self, X, y, learning_rate):
        ...
        self.weights_output += learning_rate * np.dot(self.hidden_output2.T, output_delta)
        self.bias_output += learning_rate * np.sum(output_delta, axis=0, keepdims=True)

        # Compute gradients for hidden layer 2
        hidden_error2 = np.dot(output_delta, self.weights_output.T) * self.sigmoid_derivative(self.hidden_output2)

        # Update weights and biases for hidden layer 2
        self.weights_hidden2 += learning_rate * np.dot(self.hidden_output1.T, hidden_error2)
        self.bias_hidden2 += learning_rate * np.sum(hidden_error2, axis=0, keepdims=True)

        # Compute gradients for hidden layer 1
        hidden_error1 = np.dot(hidden_error2, self.weights_hidden2.T) * self.sigmoid_derivative(self.hidden_output1)
